...
#-- export filtered dataframe: one file for each control and replicate column
filtered['SEQID'] = filtered.index
filtered = filtered.reindex(columns = ['SEQID', 'GENE'] + controlColumns + replicateColumns)
for i in range(len(controlColumns)):
    columnName = controlColumns[i]
    colArr = [columnName, 'SEQID']
    outfile = args.path + 'filtered_' + columnName + '_' + args.filename
    filtered.to_csv(outfile, columns = colArr, header = False, index = False, sep = '\t')

for i in range(len(replicateColumns)):
    columnName = replicateColumns[i]
    colArr = [columnName, 'SEQID']
    outfile = args.path + 'filtered_' + columnName + '_' + args.filename
    filtered.to_csv(outfile, columns = colArr, header = False, index = False, sep = '\t')
